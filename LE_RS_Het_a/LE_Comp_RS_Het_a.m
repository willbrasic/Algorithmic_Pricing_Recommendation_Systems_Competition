%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Logit Competitive Equilibrium Solver for Varying a_{12} and a_{21}
% William Brasic
...
% Vary the product preference matrix a's off diagonal elements.
% Define 100 equally spaced points for the second row values from 1 to 2.
a2_vector = linspace(1, 2, 100);
num_a2 = length(a2_vector);
...
% The diagonal elements remain fixed at [2, 2]. The off diagonal elements will be updated in the loop.
a = [2, 1; 
    1, 2];
...
comp_p_results = [a2_vector', zeros(num_a2, n)];
comp_d_results = [a2_vector', zeros(num_a2, n)];
comp_rvn_results = [a2_vector', zeros(num_a2, n)];
comp_pi_results = [a2_vector', zeros(num_a2, n)];
comp_cs_results = [a2_vector', zeros(num_a2, 1)]; % 1 column for CS

% Counter for storing results
index = 1;

% Iterate over each a2 value
for i = 1:num_a2

    % Update product preference matrix
    a(2, 1) = a2_vector(i);
    a(1, 2) = a2_vector(i);
