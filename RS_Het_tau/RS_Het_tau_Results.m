...
% Mean price across sellers for each unique omega and tau pair
...
% Total demand across sellers for each unique omega and tau pair
...
grid on;
xlim([0, 1]); 
ylim([min(cs_relative(:, 3) - 0.01), max(cs_relative(:, 3) + 0.01)]);
...
grid on;
xlim([0, 1]); 
ylim([min(p_sellers_mean_relative(:, 3) - 0.01), max(p_sellers_mean_relative(:, 3) + 0.01)]);
...
grid on;
xlim([0, 1]); 
ylim([min(D_sellers_relative(:, 3) - 0.01), max(D_sellers_relative(:, 3) + 0.01)]);

% Global legend 
legend_labels = [h_omega];
