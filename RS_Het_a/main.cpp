///////////////////////////////////////////////////////////////////////////
// Sellers Q-learning Varying a_{21} and a_{12} With RS
//
// William Brasic
...
// consumer types varying a_{21} and a_{12} from 1 to 2.
///////////////////////////////////////////////////////////////////////////
...
int main()
{
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        ...
        for (int a{ 0 }; a < a_values; ++a)
        {
            ...
            std::vector<std::vector<double>> d_sellers = populate_d_sellers(p_sellers, a_current, theta, c, gammma, mu, tau, a_0, n, S_sellers_cardinality, p_sellers_comb);
            std::vector<std::vector<double>> pi_sellers = populate_pi_sellers(d_sellers, p_sellers, mc, f, S_sellers_cardinality, n);
            std::vector<double> cs = populate_cs(p_sellers, a_current, theta, c, mu, gammma, tau, a_0, S_sellers_cardinality, n, p_sellers_comb);
            ...
            for (int e{ 0 }; e < E; ++e)
            {
                ...
                std::cout << "a " << a + 1 << " of " << a_values << " is: " << a_current[1][0] << "\n";
...
//                    rvn_sellers_t.reserve(maxt);
                ...
                while ((t < maxt) && (convergence_counter < norm))
                {
                    ...
                    switch (a_plat_current)
                    {
                        case 0:
                            d_sellers_t_0 = tau * (
                                    gammma * (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0)) / mu) /
                                              (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0)) / mu) + exp_a_0_mu)) +
                                    (1 - gammma) * (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0)) / mu) /
                                                    (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0)) / mu) + exp_a_0_mu))
                            ) +
                                            (1 - tau) * (
                                                    gammma * (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) /
                                                              (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                               std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                    (1 - gammma) * (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) /
                                                                    (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                                                     std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))
                                            );

                            d_sellers_t_1 = (1 - tau) * (
                                    gammma * (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                              (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                               std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                    (1 - gammma) * (std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) /
                                                    (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                                     std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))
                            );

                            cs_current = mu * (tau * (gammma * (std::log(std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0)) / mu) + exp_a_0_mu))
                                                              + (1 - gammma) * (std::log(std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0)) / mu) + exp_a_0_mu)))
                                               + (1 - tau) * (gammma * (std::log(std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) + std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu))
                                                                      + (1 - gammma) * (std::log(std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) + std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))));
                        ...
                        case 1:
                            ...
                            d_sellers_t_1 = tau * (
                                    (1 - gammma) * (std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1)) / mu) /
                                                    (std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1)) / mu) + exp_a_0_mu))
                            ) +
                                            (1 - tau) * (
                                                    gammma * (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                              (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                            ...
                            cs_current = mu * (tau * (gammma * (std::log(std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0)) / mu) + exp_a_0_mu))
                                                              + (1 - gammma) * (std::log(std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1)) / mu) + exp_a_0_mu)))
                                               + (1 - tau) * (gammma * (std::log(std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) + std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu))
                                                                      + (1 - gammma) * (std::log(std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) + std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))));
                            break;
                        ...
                        case 2:
                            ...
                            d_sellers_t_1 = tau * (
                                    gammma * (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1)) / mu) /
                                              (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1)) / mu) + exp_a_0_mu))
                            ) +
                                            (1 - tau) * (
                                                    gammma * (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                              (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                            ...
                            cs_current = mu * (tau * (gammma * (std::log(std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1)) / mu) + exp_a_0_mu))
                                                              + (1 - gammma) * (std::log(std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0)) / mu) + exp_a_0_mu)))
                                               + (1 - tau) * (gammma * (std::log(std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) + std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu))
                                                                      + (1 - gammma) * (std::log(std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) + std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))));
                            break;
                        ...
                        case 3:
                            d_sellers_t_0 =
                                    (1 - tau) * (gammma * (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) /
                                                                   (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                    std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                         (1 - gammma) * (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) /
                                                                         (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                            ...
                            d_sellers_t_1 =
                                    tau * (gammma * (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1)) / mu) /
                                                             (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1)) / mu) + exp_a_0_mu)) +
                                                   (1 - gammma) * (std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1)) / mu) /
                                                                   (std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1)) / mu) + exp_a_0_mu)))
                                    +
                                    (1 - tau) * (gammma * (std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                                   (std::exp((a_current[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                    std::exp((a_current[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                         (1 - gammma) * (std::exp((a_current[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) /
                                                                         (std::exp((a_current[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
...
//                        pi_plat_sum += pi_plat_t[j];
                ...
                for (int i{ 0 }; i < n; ++i)
                {
                    pi_sellers_store[i][e][w][a]  = pi_sellers_sum[i] / results_check;
                    rvn_sellers_store[i][e][w][a]  = rvn_sellers_sum[i] / results_check;
                    d_sellers_store[i][e][w][a]  = d_sellers_sum[i] / results_check;
                    p_sellers_store[i][e][w][a]  = p_sellers_sum[i] / results_check;
                }
                pi_plat_store[e][w][a]  = pi_plat_sum/ results_check;
                cs_store[e][w][a]  = cs_sum / results_check;
                for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                    a_plat_store[plat_action][e][w][a]  = a_plat_sum[plat_action] / results_check;
...
//                std::vector<double> rvn_sellers_sum(n, 0.0);
            ...
            for (int e{ 0 }; e < E; ++e)
            {
                for (int i{ 0 }; i < n; ++i)
                {
                    pi_sellers_sum[i] += pi_sellers_store[i][e][w][a] ;
//                        rvn_sellers_sum[i] += rvn_sellers_store[i][e][w][a] ;
                    d_sellers_sum[i] += d_sellers_store[i][e][w][a] ;
                    p_sellers_sum[i] += p_sellers_store[i][e][w][a];
                }
                converge_sum += converge_store[e][w][a];
//                    pi_plat_sum += pi_plat_store[e][w][a];
                cs_sum += cs_store[e][w][a];
                for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                    a_plat_sum[plat_action] += a_plat_store[plat_action][e][w][a];
            }
            for (int i{ 0 }; i < n; ++i)
            {
                pi_sellers_store_avg_e[i][w][a] = pi_sellers_sum[i] / E;
//                    rvn_sellers_store_avg_e[i][w][a] = rvn_sellers_sum[i] / E;
                d_sellers_store_avg_e[i][w][a] = d_sellers_sum[i] / E;
                p_sellers_store_avg_e[i][w][a] = p_sellers_sum[i] / E;
            }
            converge_store_avg_e[w][a] = converge_sum / E;
//                pi_plat_store_avg_e[w][a] = pi_plat_sum/ E;
            cs_store_avg_e[w][a] = cs_sum / E;
            for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                a_plat_store_avg_e[plat_action][w][a] = a_plat_sum[plat_action] / E;
    ...
    // Writing headers
    pi_sellers_file << "omega,a,firm,profit\n";
    d_sellers_file << "omega,a,firm,demand\n";
    p_sellers_file << "omega,a,firm,price\n";
    cs_file << "omega,a,cs\n";
    converge_file << "omega,a,t\n";
    a_plat_file << "omega,a,action,Count\n";
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        for (int a{ 0 }; a < a_values; ++a)
        {
            std::vector<std::vector<double>> a_current = a_vector[a];
            for (int i{0}; i < n; ++i) {
                pi_sellers_file << omega[w] << "," << a_current[1][0] << "," << i + 1 << ","
                                << pi_sellers_store_avg_e[i][w][a] << "\n";
                d_sellers_file << omega[w] << "," << a_current[1][0] << "," << i + 1 << ","
                               << d_sellers_store_avg_e[i][w][a] << "\n";
                p_sellers_file << omega[w] << "," << a_current[1][0] << "," << i + 1 << ","
                               << p_sellers_store_avg_e[i][w][a] << "\n";
            }
            cs_file << omega[w] << "," << a_current[1][0] << "," << cs_store_avg_e[w][a] << "\n";
            converge_file << omega[w] << "," << a_current[1][0] << "," << converge_store_avg_e[w][a]
                          << "\n";
            for (int j{0}; j < plat_actions; ++j) {
                a_plat_file << omega[w] << "," << a_current[1][0] << "," << j + 1 << ","
                            << a_plat_store_avg_e[j][w][a] << "\n";
            }
        }
    }
