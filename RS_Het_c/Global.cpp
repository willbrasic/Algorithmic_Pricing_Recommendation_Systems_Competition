...
namespace Global {
    ...
    const int c_values { 100 };

    // Minimum and maximum values for c
    const double c_min { 0.0 };
    const double c_max { 1.0 / 2.0 };

    // Step size for c
    const double c_step_size = (c_max - c_min) / (c_values - 1);
    ...
    std::vector<std::vector<std::vector<std::vector<double>>>> a_plat_store(
            ...
            std::vector<std::vector<std::vector<double>>>(
                    ...
                    std::vector<std::vector<double>>(
                            omega.size(),
                            std::vector<double>(c_values, 0.0)
                    )
            )
    );
