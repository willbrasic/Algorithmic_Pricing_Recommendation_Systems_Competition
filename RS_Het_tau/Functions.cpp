...
{
    // Create vector to store values of tau
    std::vector<double> tau_vector(tau_values, 0.0);

    // Populate vector with *tau_values* equally spaced points
    for (int tau{ 0 }; tau < tau_values; ++tau)
        tau_vector[tau] = tau_min + (tau * tau_step_size);
