%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Increased f Relative to Baseine Specification With RS Results
% William Brasic 
...
% January 2024
%
% This script obtains results for RS_Het_f.m and No_RS_Het_f.m
% 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Without RS Logit Equilibrium 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Logit version
logit_version = "No_" + version;
...
% Version of simulation without RS
version_no_rs = "No_" + version;

% Add path to without RS files
addpath(strcat(version_no_rs));

% File name for final results averaged over episodes
results_final_no_rs_file_name = strcat(version_no_rs, "\", ...
    version_no_rs, "_Results_Final_", num2str(R), ".csv");

% File name for learning curve results for sellers' prices
p_sellers_lc_no_rs_file_name = strcat(version_no_rs, "\", ...
    version_no_rs, "_LC_Prices_Sellers_", num2str(R), ".mat");

% File name for learning curve results for sellers' revenues
rvn_sellers_lc_no_rs_file_name = strcat(version_no_rs, "\", ...
    version_no_rs, "_LC_Revenues_Sellers_", num2str(R), ".mat");

% File name for learning curve results for sellers' demand
d_sellers_lc_no_rs_file_name = strcat(version_no_rs, "\", ...
    version_no_rs, "_LC_Demand_Sellers_", num2str(R), ".mat");

% File name for learning curve results for sellers' profits
pi_sellers_lc_no_rs_file_name = strcat(version_no_rs, "\", ...
    version_no_rs, "_LC_Profits_Sellers_", num2str(R), ".mat");

% File name for learning curve results for consumer surplus
cs_lc_no_rs_file_name = strcat(version_no_rs, "\", ...
    version_no_rs, "_LC_CS_", num2str(R), ".mat");

% File name for time steps until convergence
converge_no_rs_file_name = strcat(version_no_rs, "\", ...
    version_no_rs, "_Converge_", num2str(R), ".mat");
...
results_final_no_rs = readtable(results_final_no_rs_file_name);

% Mean convergence            
mean_converge_no_rs = results_final_no_rs.Mean_Convergence;

% Mean market and firm profits
mean_market_pi_no_rs = results_final_no_rs.Mean_Market_Profit;
mean_firm_pi_no_rs = [results_final_no_rs.Mean_Firm_1_Profit; results_final_no_rs.Mean_Firm_2_Profit];

% Mean market and firm revenues
mean_market_rvn_no_rs = results_final_no_rs.Mean_Market_Revenue;
mean_firm_rvn_no_rs = [results_final_no_rs.Mean_Firm_1_Revenue; results_final_no_rs.Mean_Firm_2_Revenue];

% Mean market and firm demand
mean_market_d_no_rs = results_final_no_rs.Mean_Market_Quantity;
mean_firm_d_no_rs = [results_final_no_rs.Mean_Firm_1_Quantity; results_final_no_rs.Mean_Firm_2_Quantity];

% Mean market and firm prices
mean_market_p_no_rs = results_final_no_rs.Mean_Market_Price;
mean_firm_p_no_rs = [results_final_no_rs.Mean_Firm_1_Price; results_final_no_rs.Mean_Firm_2_Price];

% Mean consumer surplus
mean_cs_no_rs = results_final_no_rs.Mean_CS;
...
fprintf(1,"            %1.0f", [1:n]')
fprintf(1,"\n---------------------------------------------------------\n");
fprintf(1,"\nProfits            %1.4f", mean_market_pi_no_rs)
fprintf(1,"      %1.4f", mean_firm_pi_no_rs)
fprintf(1,"\nRevnues            %1.4f", mean_market_rvn_no_rs)
fprintf(1,"      %1.4f", mean_firm_rvn_no_rs)
fprintf(1,"\nDemand             %1.4f", mean_market_d_no_rs)
fprintf(1,"      %1.4f", mean_firm_d_no_rs)
fprintf(1,"\nPrices             %1.4f", mean_market_p_no_rs)
fprintf(1,"      %1.4f", mean_firm_p_no_rs)
fprintf(1,"\nCS                 %1.4f", mean_cs_no_rs)
fprintf(1,"\n---------------------------------------------------------\n");
...
fprintf(1,"        %1.0f", [1:n]')
fprintf(1,"\n---------------------------------------------------------\n");
fprintf(1,"\nProfits         %1.2f%%", 100 * (mean_market_pi_no_rs - sum(comp_pi))/sum(comp_pi))
fprintf(1,"    %1.2f%%", 100 * (mean_firm_pi_no_rs - comp_pi')/comp_pi')
fprintf(1,"\nRevenues        %1.2f%%", 100 * (mean_market_rvn_no_rs - sum(comp_rvn)) / sum(comp_rvn))
fprintf(1,"     %1.2f%%", 100 * (mean_firm_rvn_no_rs - comp_rvn')/comp_rvn')
fprintf(1,"\nDemand          %1.2f%%", 100 * (mean_market_d_no_rs - sum(comp_d))/sum(comp_d))
fprintf(1,"    %1.2f%%", 100 * (mean_firm_d_no_rs - comp_d')/comp_d')
fprintf(1,"\nPrices          %1.2f%%", 100 * (mean_market_p_no_rs - mean(comp_p))/mean(comp_p))
fprintf(1,"    %1.2f%%", 100 * (mean_firm_p_no_rs - comp_p')/comp_p')
fprintf(1,"\nCS              %1.2f%%", 100 * (mean_cs_no_rs - comp_cs)/comp_cs)
fprintf(1,"\n---------------------------------------------------------\n");
...
xticklabels({"1", "300", "600", "900", "1200"});
xlabel("Time Period \times 1000");
grid on;
