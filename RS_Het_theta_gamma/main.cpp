...
int main()
{
    ...
    std::vector<double> gammma_vector = populate_gammma(gammma_min, gammma_step_size, gammma_values);

    // Populate gammma vector
    std::vector<double> theta_1_vector = populate_theta_1(theta_1_min, theta_1_step_size, theta_1_values);
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        ...
        // Loop over values for gammma
        for (int g{ 0 }; g < gammma_vector.size(); ++g)
        {
            // Current value of gammma
            double gammma_current = gammma_vector[g];
            ...
            for (int theta_1{ 0 }; theta_1 < theta_1_vector.size(); ++theta_1)
            {
                // Current value of theta_1
                double theta_1_current = theta_1_vector[theta_1];

                // Create current theta vector
                std::vector<double> theta = {theta_1_current, 1.0};

                // Populate initial values to obtain initial Q-matrices for sellers and the platform
                std::vector<std::vector<double>> d_sellers = populate_d_sellers(p_sellers, a, theta, c, gammma_current, mu, tau, a_0, n, S_sellers_cardinality, p_sellers_comb);
                std::vector<std::vector<double>> pi_sellers = populate_pi_sellers(d_sellers, p_sellers, mc, f, S_sellers_cardinality, n);
                std::vector<double> cs = populate_cs(p_sellers, a, theta, c, mu, gammma_current, tau, a_0, S_sellers_cardinality, n, p_sellers_comb);
                ...
                for (int e{ 0 }; e < E; ++e)
                {
                    ...
                    // Print the current value of gammma
                    std::cout << "gammma " << g + 1 << " of " << gammma_vector.size() << " is: " << gammma_current << "\n";

                    // Print the current value of theta_1
                    std::cout << "theta_1 " << theta_1 + 1 << " of " << theta_1_vector.size() << " is: " << theta_1_current << "\n";
...
//                    d_sellers_t.reserve(maxt);
                    ...
                    while ((t < maxt) && (convergence_counter < norm))
                    {
                        ...
                        switch (a_plat_current)
                        {
                            case 0:
                                d_sellers_t_0 = tau * (
                                        gammma_current * (std::exp((a[0][0] - (theta[0] * p_sellers_t_0)) / mu) /
                                                  (std::exp((a[0][0] - (theta[0] * p_sellers_t_0)) / mu) + exp_a_0_mu)) +
                                        (1 - gammma_current) * (std::exp((a[0][1] - (theta[1] * p_sellers_t_0)) / mu) /
                                                        (std::exp((a[0][1] - (theta[1] * p_sellers_t_0)) / mu) + exp_a_0_mu))
                                ) +
                                                (1 - tau) * (
                                                        gammma_current * (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) /
                                                                  (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                   std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                        (1 - gammma_current) * (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) /
                                                                        (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                                                         std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))
                                                );

                                d_sellers_t_1 = (1 - tau) * (
                                        gammma_current * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                  (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                   std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                        (1 - gammma_current) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) /
                                                        (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                                         std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))
                                );
                            ...
                            case 1:
                                ...
                                d_sellers_t_1 = tau * (
                                        (1 - gammma_current) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1)) / mu) /
                            ...
                            case 2:
                                ...
                                d_sellers_t_1 = tau * (
                                        gammma_current * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1)) / mu) /
                            ...
                            case 3:
                                d_sellers_t_0 =
                                        (1 - tau) * (gammma_current * (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) /
                                                                       (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                        std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                             (1 - gammma_current) * (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) /
                                                                             (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                ...
                                d_sellers_t_1 =
                                        tau * (gammma_current * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1)) / mu) /
                                                                 (std::exp((a[1][0] - (theta[0] * p_sellers_t_1)) / mu) + exp_a_0_mu)) +
                                                       (1 - gammma_current) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1)) / mu) /
                                                                       (std::exp((a[1][1] - (theta[1] * p_sellers_t_1)) / mu) + exp_a_0_mu)))
                                        +
                                        (1 - tau) * (gammma_current * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                                       (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                        std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                             (1 - gammma_current) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) /
                                                                             (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
...
//                        pi_plat_t.emplace_back(pi_plat_current);
                    ...
                    converge_store[e][w][g][theta_1] = t - 1;
...
//                        pi_plat_sum += pi_plat_t[j];
                    ...
                    for (int i{ 0 }; i < n; ++i)
                    {
                        pi_sellers_store[i][e][w][g][theta_1]  = pi_sellers_sum[i] / results_check;
                        rvn_sellers_store[i][e][w][g][theta_1]  = rvn_sellers_sum[i] / results_check;
                        d_sellers_store[i][e][w][g][theta_1]  = d_sellers_sum[i] / results_check;
                        p_sellers_store[i][e][w][g][theta_1]  = p_sellers_sum[i] / results_check;
                    }
                    pi_plat_store[e][w][g][theta_1]  = pi_plat_sum/ results_check;
                    cs_store[e][w][g][theta_1]  = cs_sum / results_check;
                    for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                        a_plat_store[plat_action][e][w][g][theta_1]  = a_plat_sum[plat_action] / results_check;
...
//                std::vector<double> d_sellers_sum(n, 0.0);
                ...
                for (int e{ 0 }; e < E; ++e)
                {
                    for (int i{ 0 }; i < n; ++i)
                    {
                        pi_sellers_sum[i] += pi_sellers_store[i][e][w][g][theta_1] ;
//                        rvn_sellers_sum[i] += rvn_sellers_store[i][e][w][g][theta_1] ;
//                        d_sellers_sum[i] += d_sellers_store[i][e][w][g][theta_1] ;
                        p_sellers_sum[i] += p_sellers_store[i][e][w][g][theta_1];
                    }
                    converge_sum += converge_store[e][w][g][theta_1];
//                    pi_plat_sum += pi_plat_store[e][w][g][theta_1];
                    cs_sum += cs_store[e][w][g][theta_1];
                    for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                        a_plat_sum[plat_action] += a_plat_store[plat_action][e][w][g][theta_1];
                }
                for (int i{ 0 }; i < n; ++i)
                {
                    pi_sellers_store_avg_e[i][w][g][theta_1] = pi_sellers_sum[i] / E;
//                    rvn_sellers_store_avg_e[i][w][g][theta_1] = rvn_sellers_sum[i] / E;
//                    d_sellers_store_avg_e[i][w][g][theta_1] = d_sellers_sum[i] / E;
                    p_sellers_store_avg_e[i][w][g][theta_1] = p_sellers_sum[i] / E;
                }
                converge_store_avg_e[w][g][theta_1] = converge_sum / E;
//                pi_plat_store_avg_e[w][g][theta_1] = pi_plat_sum/ E;
                cs_store_avg_e[w][g][theta_1] = cs_sum / E;
                for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                    a_plat_store_avg_e[plat_action][w][g][theta_1] = a_plat_sum[plat_action] / E;
    ...
    std::string version = "RS_Het_theta_gamma";
...
//    pi_plat_file.open(path_prefix + version + "_Results/" + version + "_Profits_Plat.csv");
    ...
    // Writing headers
    pi_sellers_file << "omega,gamma,theta_1,firm,profit\n";
//    rvn_sellers_file << "omega,gamma,theta_1,firm,revenue\n";
//    d_sellers_file << "omega,gamma,theta_1,firm,demand\n";
    p_sellers_file << "omega,gamma,theta_1,firm,price\n";
    cs_file << "omega,gamma,theta_1,cs\n";
    converge_file << "omega,gamma,theta_1,t\n";
//    pi_plat_file << "omega,gamma,theta_1,profit\n";
    a_plat_file << "omega,gamma,theta_1,action,Count\n";
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        for (int g{ 0 }; g < gammma_vector.size(); ++g)
        {
            ...
            {
                for (int i{0}; i < n; ++i) {
                    pi_sellers_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << i + 1 << ","
                                    << pi_sellers_store_avg_e[i][w][g][theta_1] << "\n";
//                    rvn_sellers_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << i + 1 << ","
//                                     << rvn_sellers_store_avg_e[i][w][g][theta_1] << "\n";
//                    d_sellers_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << i + 1 << ","
//                                   << d_sellers_store_avg_e[i][w][g][theta_1] << "\n";
                    p_sellers_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << i + 1 << ","
                                   << p_sellers_store_avg_e[i][w][g][theta_1] << "\n";
                }
                cs_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << cs_store_avg_e[w][g][theta_1] << "\n";
                converge_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << converge_store_avg_e[w][g][theta_1]
                              << "\n";
//                pi_plat_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << pi_plat_store_avg_e[w][g][theta_1]
//                             << "\n";
                for (int j{0}; j < plat_actions; ++j) {
                    a_plat_file << omega[w] << "," << gammma_vector[g] << "," << theta_1_vector[theta_1] << "," << j + 1 << ","
                                << a_plat_store_avg_e[j][w][g][theta_1] << "\n";
                }
            }
        }
    }
