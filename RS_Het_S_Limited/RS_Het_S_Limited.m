...
% preferences. The state space for all agents is reduced so past period
% seller prices. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Specify logit equilibrium version
...
% Sellers demand and overall cs in each state 
...
% Average profits across states when seller *i* sets price *j* 
for i = 1:n 
    for j = 1:m
...
for w = 1:length(omega)
    for plat_action = 1:plat_actions
        Q_plat_0(plat_action, w) = mean(pi_plat(:, w)) / (1 - delta);
    end
end
...
for w = 1:length(omega)
    ...
    for e = 1:E
        ...
        s_sellers(1) = temp_seller;

        % Determine initial state for platform
        s_plat(1) = temp_seller;
        ...
        while ((t < maxt) && (convergence_count < norm))
            ...
            s_sellers(t + 1) = temp_seller;
            s_plat(t + 1) = temp_seller;
