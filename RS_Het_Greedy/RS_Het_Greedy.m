...
% preferences. The platform's RS uses the epsilon-greedy action
% selection mechanism rather than UCB.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for w = 1:length(omega)
    ...
    for e = 1:E
        ...
        while ((t < maxt) && (convergence_count < norm))
            ...
            % Explore for platform
            if rand(1, 1) < exp(-beta * t) 
                a_plat(t) = randperm(plat_actions, 1);
            % Exploit for platform
            else
                a_plat(t) = find(Q_plat(s_plat(t), :, e) == max(Q_plat(s_plat(t), :, e)), 1); 
            end
