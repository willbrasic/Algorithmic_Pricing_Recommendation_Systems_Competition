...
// using a Q-learning recommendation system. Price sensitivities and
...
int main()
{
    ...
    std::vector<double> alpha_vector = populate_alpha(alpha_min, alpha_step_size, alpha_values);

    // Populate beta vector
    std::vector<double> beta_vector = populate_beta(beta_min, beta_step_size, beta_values);
    ...
    std::vector<std::vector<double>> d_sellers = populate_d_sellers(p_sellers, a, theta, c, gammma, mu, tau, a_0, n, S_sellers_cardinality, p_sellers_comb);
    std::vector<std::vector<double>> pi_sellers = populate_pi_sellers(d_sellers, p_sellers, mc, f, S_sellers_cardinality, n);
    std::vector<double> cs = populate_cs(p_sellers, a, theta, c, mu, gammma, tau, a_0, S_sellers_cardinality, n, p_sellers_comb);
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        ...
        // Loop over values for alpha
        for (int alpha{ 0 }; alpha < alpha_vector.size(); ++alpha)
        {
            // Current value of alpha
            double alpha_current = alpha_vector[alpha];

            ////////////////////////////////////
            // beta Loop
            ////////////////////////////////////

            // Loop over values for beta
            for (int beta{ 0 }; beta < beta_vector.size(); ++beta)
            {
                // Current value of beta
                double beta_current = beta_vector[beta];

                // Get exploration number for all possible time steps
                std::vector<double> minus_beta_times_t(maxt, 0.0);
                for (int t{ 0 }; t < maxt; ++t)
                    minus_beta_times_t[t] = exp(-beta_current * t);
                ...
                for (int e{ 0 }; e < E; ++e)
                {
                    ...
                    std::cout << "alpha " << alpha + 1 << " of " << alpha_vector.size() << " is: " << alpha_current << "\n";

                    // Print the current value of beta
                    std::cout << "beta " << beta + 1 << " of " << beta_vector.size() << " is: " << beta_current << "\n";
...
//                    d_sellers_t.reserve(maxt);
                    ...
                    while ((t < maxt) && (convergence_counter < norm))
                    {
                        ...
                        switch (a_plat_current)
                        {
                            case 0:
                                d_sellers_t_0 = tau * (
                                        ...
                                        (1 - gammma) * (std::exp((a[0][1] - (theta[1] * p_sellers_t_0)) / mu) /
                                                ...
                                                (1 - tau) * (
                                                                    gammma * (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) /
                                                                          (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                           std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                        (1 - gammma) * (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) /
                                                                                (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                                                                 std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))
                                                );

                                d_sellers_t_1 = (1 - tau) * (
                                                                    gammma * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                          (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                           std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                        (1 - gammma) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) /
                                                                (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                                                 std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))
                                );
                                ...
                                cs_current = mu * (tau * (gammma * (std::log(std::exp((a[0][0] - (theta[0] * p_sellers_t_0)) / mu) + exp_a_0_mu))
                                                   ...
                                                   + (1 - tau) * (gammma * (std::log(std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) + std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu))
                                                                  + (1 - gammma) * (std::log(std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) + std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))));
                            ...
                            case 3:
                                d_sellers_t_0 =
                                        (1 - tau) * (gammma * (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) /
                                                                       (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                        std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                     (1 - gammma) * (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) /
                                                                             (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                                ...
                                d_sellers_t_1 =
                                        tau * (gammma * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1)) / mu) /
                                               ...
                                               (1 - gammma) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1)) / mu) /
                                                                       (std::exp((a[1][1] - (theta[1] * p_sellers_t_1)) / mu) + exp_a_0_mu)))
                                        +
                                        (1 - tau) * (gammma * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                                       (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                        std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                     (1 - gammma) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) /
                                                                             (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
...
//                        rvn_sellers_t.emplace_back(std::vector<double>{rvn_sellers_t_0, rvn_sellers_t_1});
                        ...
                        double pi_plat_current = omega_current * (f * (p_sellers_t_0 * d_sellers_t_0 + p_sellers_t_1 * d_sellers_t_1)) + (1 - omega_current) * cs_current;
//                        pi_plat_t.emplace_back(pi_plat_current);
                        ...
                        for (int i{ 0 }; i < n; ++i)
                        {
                            ...
                            Q_sellers[s_sellers_current][a_sellers_t[t][i]][i] = (1 - alpha_current) * Q_sellers[s_sellers_current][a_sellers_t[t][i]][i]
                                                                                 + alpha_current * (pi_sellers_t[t][i] + delta * max_Q_sellers);
                        }

                        // Platform's Q-learning update
                        double max_Q_plat = *std::max_element(Q_plat[s_plat_next].begin(), Q_plat[s_plat_next].end());
                        Q_plat[s_plat_current][a_plat_current] = (1 - 0.15) * Q_plat[s_plat_current][a_plat_current]
                                                                 + 0.15 * (pi_plat_current + delta * max_Q_plat);
                    ...
                    converge_store[e][w][alpha][beta] = t - 1;
...
//                        pi_plat_sum += pi_plat_t[j];
                    ...
                    for (int i{ 0 }; i < n; ++i)
                    {
                        pi_sellers_store[i][e][w][alpha][beta]  = pi_sellers_sum[i] / results_check;
                        rvn_sellers_store[i][e][w][alpha][beta]  = rvn_sellers_sum[i] / results_check;
                        d_sellers_store[i][e][w][alpha][beta]  = d_sellers_sum[i] / results_check;
                        p_sellers_store[i][e][w][alpha][beta]  = p_sellers_sum[i] / results_check;
                    }
                    pi_plat_store[e][w][alpha][beta]  = pi_plat_sum/ results_check;
                    cs_store[e][w][alpha][beta]  = cs_sum / results_check;
                    for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                        a_plat_store[plat_action][e][w][alpha][beta]  = a_plat_sum[plat_action] / results_check;
...
//                std::vector<double> d_sellers_sum(n, 0.0);
                ...
                for (int e{ 0 }; e < E; ++e)
                {
                    for (int i{ 0 }; i < n; ++i)
                    {
                        pi_sellers_sum[i] += pi_sellers_store[i][e][w][alpha][beta] ;
//                        rvn_sellers_sum[i] += rvn_sellers_store[i][e][w][alpha][beta] ;
//                        d_sellers_sum[i] += d_sellers_store[i][e][w][alpha][beta] ;
                        p_sellers_sum[i] += p_sellers_store[i][e][w][alpha][beta];
                    }
                    converge_sum += converge_store[e][w][alpha][beta];
//                    pi_plat_sum += pi_plat_store[e][w][alpha][beta];
                    cs_sum += cs_store[e][w][alpha][beta];
                    for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                        a_plat_sum[plat_action] += a_plat_store[plat_action][e][w][alpha][beta];
                }
                for (int i{ 0 }; i < n; ++i)
                {
                    pi_sellers_store_avg_e[i][w][alpha][beta] = pi_sellers_sum[i] / E;
//                    rvn_sellers_store_avg_e[i][w][alpha][beta] = rvn_sellers_sum[i] / E;
//                    d_sellers_store_avg_e[i][w][alpha][beta] = d_sellers_sum[i] / E;
                    p_sellers_store_avg_e[i][w][alpha][beta] = p_sellers_sum[i] / E;
                }
                converge_store_avg_e[w][alpha][beta] = converge_sum / E;
//                pi_plat_store_avg_e[w][alpha][beta] = pi_plat_sum/ E;
                cs_store_avg_e[w][alpha][beta] = cs_sum / E;
                for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                    a_plat_store_avg_e[plat_action][w][alpha][beta] = a_plat_sum[plat_action] / E;
    ...
    std::ofstream pi_sellers_file, p_sellers_file, cs_file, converge_file, a_plat_file;

    // Set correct file path prefix
    std::string path_prefix = HPC ? "./" : "../";
    std::string version = "RS_Het_alpha_beta_Heat_omega_1";

    // Open files
    pi_sellers_file.open(path_prefix + version + "_Results/" + version + "_Profits_Sellers.csv");
//    rvn_sellers_file.open(path_prefix + version + "_Results/" + version + "_Revenues_Sellers.csv");
...
//    pi_plat_file.open(path_prefix + version + "_Results/" + version + "_Profits_Plat.csv");
    a_plat_file.open(path_prefix + version + "_Results/" + version + "_Actions_Plat.csv");

    // Writing headers
    pi_sellers_file << "omega,alpha,beta,firm,profit\n";
//    rvn_sellers_file << "omega,alpha,beta,firm,revenue\n";
//    d_sellers_file << "omega,alpha,beta,firm,demand\n";
    p_sellers_file << "omega,alpha,beta,firm,price\n";
    cs_file << "omega,alpha,beta,cs\n";
    converge_file << "omega,alpha,beta,t\n";
//    pi_plat_file << "omega,alpha,beta,profit\n";
    a_plat_file << "omega,alpha,beta,action,Count\n";
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        for (int alpha{ 0 }; alpha < alpha_vector.size(); ++alpha)
        {
            ...
            {
                for (int i{0}; i < n; ++i) {
                    pi_sellers_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << i + 1 << ","
                                    << pi_sellers_store_avg_e[i][w][alpha][beta] << "\n";
//                    rvn_sellers_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << i + 1 << ","
//                                     << rvn_sellers_store_avg_e[i][w][alpha][beta] << "\n";
//                    d_sellers_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << i + 1 << ","
//                                   << d_sellers_store_avg_e[i][w][alpha][beta] << "\n";
                    p_sellers_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << i + 1 << ","
                                   << p_sellers_store_avg_e[i][w][alpha][beta] << "\n";
                }
                cs_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << cs_store_avg_e[w][alpha][beta] << "\n";
                converge_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << converge_store_avg_e[w][alpha][beta]
                              << "\n";
//                pi_plat_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << pi_plat_store_avg_e[w][alpha][beta]
//                             << "\n";
                for (int j{0}; j < plat_actions; ++j) {
                    a_plat_file << omega[w] << "," << alpha_vector[alpha] << "," << beta_vector[beta] << "," << j + 1 << ","
                                << a_plat_store_avg_e[j][w][alpha][beta] << "\n";
                }
            }
        }
    }
