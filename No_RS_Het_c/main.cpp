///////////////////////////////////////////////////////////////////////////
// Sellers Q-learning Varying c No RS
//
// William Brasic
...
int main()
{
    ...
    for(int c{ 0 }; c < c_values; ++c)
    {
        ...
        std::vector<std::vector<double>> d_sellers = populate_d_sellers(p_sellers, a, theta, c_current, gammma, mu, a_0, n, S_sellers_cardinality);
        std::vector<std::vector<double>> pi_sellers = populate_pi_sellers(d_sellers, p_sellers, mc, S_sellers_cardinality, n, f);
        std::vector<double> cs = populate_cs(p_sellers, a, theta, c_current, mu, gammma, a_0, S_sellers_cardinality, n, k);
        ...
        for (int e{ 0 }; e < E; ++e)
        {
            ...
            while ((t < maxt) && (convergence_counter < norm))
            {
                ...
                d_sellers_t_0 =
                        gammma * (exp((a[0][0] - theta[0] * p_sellers_t_0 - c_current[0]) / mu) /
                                         (exp((a[0][0] - theta[0] * p_sellers_t_0 - c_current[0]) / mu) +
                                          exp((a[1][0] - theta[0] * p_sellers_t_1 - c_current[0]) / mu) + exp_a_0_mu)) +
                        (1 - gammma) * (exp((a[0][1] - theta[1] * p_sellers_t_0 - c_current[1]) / mu) /
                                               (exp((a[0][1] - theta[1] * p_sellers_t_0 - c_current[1]) / mu) +
                                                exp((a[1][1] - theta[1] * p_sellers_t_1 - c_current[1]) / mu) + exp_a_0_mu));
                d_sellers_t_1 =
                        gammma * (exp((a[1][0] - theta[0] * p_sellers_t_1 - c_current[0]) / mu) /
                                         (exp((a[0][0] - theta[0] * p_sellers_t_0 - c_current[0]) / mu) +
                                          exp((a[1][0] - theta[0] * p_sellers_t_1 - c_current[0]) / mu) + exp_a_0_mu)) +
                        (1 - gammma) * (exp((a[1][1] - theta[1] * p_sellers_t_1 - c_current[1]) / mu) /
                                               (exp((a[0][1] - theta[1] * p_sellers_t_0 - c_current[1]) / mu) +
                                                exp((a[1][1] - theta[1] * p_sellers_t_1 - c_current[1]) / mu) + exp_a_0_mu));
                d_sellers_t.emplace_back(std::vector<double>{d_sellers_t_0, d_sellers_t_1});
                
                // Consumer surplus at time period *t*
                double exp_sum_gamma = exp((a[0][0] - theta[0] * p_sellers_t_0 - c_current[0]) / mu) + exp((a[1][0] - theta[0] * p_sellers_t_1 - c_current[0]) / mu) + exp_a_0_mu;
                double exp_sum_one_minus_gamma = exp((a[0][1] - theta[1] * p_sellers_t_0 - c_current[1]) / mu) + exp((a[1][1] - theta[1] * p_sellers_t_1 - c_current[1]) / mu) + exp_a_0_mu;
...
//                        pi_plat_sum += pi_plat_t[j];
            ...
            for (int i{ 0 }; i < n; ++i)
            {
                pi_sellers_store[i][e][c]  = pi_sellers_sum[i] / results_check;
                rvn_sellers_store[i][e][c]  = rvn_sellers_sum[i] / results_check;
                d_sellers_store[i][e][c]  = d_sellers_sum[i] / results_check;
                p_sellers_store[i][e][c]  = p_sellers_sum[i] / results_check;
            }
            cs_store[e][c]  = cs_sum / results_check;
...
//                std::vector<double> rvn_sellers_sum(n, 0.0);
        ...
        for (int e{ 0 }; e < E; ++e)
        {
            for (int i{ 0 }; i < n; ++i)
            {
                pi_sellers_sum[i] += pi_sellers_store[i][e][c] ;
//                        rvn_sellers_sum[i] += rvn_sellers_store[i][e][c] ;
                d_sellers_sum[i] += d_sellers_store[i][e][c] ;
                p_sellers_sum[i] += p_sellers_store[i][e][c];
            }
            converge_sum += converge_store[e][c];
//                    pi_plat_sum += pi_plat_store[e][c];
            cs_sum += cs_store[e][c];
        }
        for (int i{ 0 }; i < n; ++i)
        {
            pi_sellers_store_avg_e[i][c] = pi_sellers_sum[i] / E;
//                    rvn_sellers_store_avg_e[i][c] = rvn_sellers_sum[i] / E;
            d_sellers_store_avg_e[i][c] = d_sellers_sum[i] / E;
            p_sellers_store_avg_e[i][c] = p_sellers_sum[i] / E;
        }
        converge_store_avg_e[c] = converge_sum / E;
//                pi_plat_store_avg_e[c] = pi_plat_sum/ E;
        cs_store_avg_e[c] = cs_sum / E;
    ...
    std::string version = "No_RS_Het_c";
    ...
    // Writing headers
    pi_sellers_file << "c,firm,profit\n";
    d_sellers_file << "c,firm,demand\n";
    p_sellers_file << "c,firm,price\n";
    cs_file << "c,cs\n";
    converge_file << "c,t\n";
    ...
    for (int c{ 0 }; c < c_values; ++c)
    {
        ...
        for (int i{0}; i < n; ++i) {
            pi_sellers_file << c_current << "," << i + 1 << "," << pi_sellers_store_avg_e[i][c] << "\n";
            d_sellers_file << c_current << "," << i + 1 << "," << d_sellers_store_avg_e[i][c] << "\n";
            p_sellers_file << c_current << "," << i + 1 << "," << p_sellers_store_avg_e[i][c] << "\n";
        }
        cs_file << c_current << "," << cs_store_avg_e[c] << "\n";
        converge_file << c_current << "," << converge_store_avg_e[c] << "\n";
    }
