...
)
{
    ...
    for (int j{ 0 }; j < S_sellers_cardinality; ++j)
    {
        for (int i{ 0 }; i < n; ++i)
        {
            pi_sellers[j][i] = ((1 - f[i]) * p_sellers[j][i] - mc[i]) * d_sellers[j][i];
        }
    }
...
)
{
    std::vector<std::vector<std::vector<double>>> result(
            a_values,
            std::vector<std::vector<double>>(2, std::vector<double>(2, 2.0))
    );

    for (int a = 0; a < a_values; ++a) {
        double new_val = a_min + a * a_step_size;
        for (int col = 0; col < k; ++col) {
            result[a][1][col] = new_val;
        }
    }
