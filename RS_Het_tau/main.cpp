...
// using a Q-learning recommendation system. The tau parameter is varied
// from 0 to 1.
///////////////////////////////////////////////////////////////////////////
...
int main()
{
    ...
    std::vector<double> tau_vector = populate_tau(tau_min, tau_step_size, tau_values);
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        ...
        // Loop over values for tau
        for (int tau{ 0 }; tau < tau_values; ++tau)
        {
            
            // Current value of tau
            double tau_current = tau_vector[tau];

            // Populate initial values to obtain initial Q-matrices for sellers and the platform
            std::vector<std::vector<double>> d_sellers = populate_d_sellers(p_sellers, a, theta, c, gammma, mu, tau_current, a_0, n, S_sellers_cardinality, p_sellers_comb);
            std::vector<std::vector<double>> pi_sellers = populate_pi_sellers(d_sellers, p_sellers, mc, f, S_sellers_cardinality, n);
            std::vector<double> cs = populate_cs(p_sellers, a, theta, c, mu, gammma, tau_current, a_0, S_sellers_cardinality, n, p_sellers_comb);
            ...
            for (int e{ 0 }; e < E; ++e)
            {
                ...
                // Print the current value of tau
                std::cout << "tau " << tau + 1 << " of " << tau_values << " is: " << tau_current << "\n";
                ...
                while ((t < maxt) && (convergence_counter < norm))
                {
                    ...
                    switch (a_plat_current)
                    {
                        case 0:
                            ...
                            cs_current = mu * (tau_current * (gammma * (std::log(std::exp((a[0][0] - (theta[0] * p_sellers_t_0)) / mu) + exp_a_0_mu))
                                                              + (1 - gammma) * (std::log(std::exp((a[0][1] - (theta[1] * p_sellers_t_0)) / mu) + exp_a_0_mu)))
                                               + (1 - tau_current) * (gammma * (std::log(std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) + std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu))
                        ...
                        case 2:
                            ...
                            cs_current = mu * (tau_current * (gammma * (std::log(std::exp((a[1][0] - (theta[0] * p_sellers_t_1)) / mu) + exp_a_0_mu))
                                                              + (1 - gammma) * (std::log(std::exp((a[0][1] - (theta[1] * p_sellers_t_0)) / mu) + exp_a_0_mu)))
                                               + (1 - tau_current) * (gammma * (std::log(std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) + std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu))
                                                                      + (1 - gammma) * (std::log(std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) + std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) + exp_a_0_mu))));
                            break;
                        ...
                        case 3:
                            d_sellers_t_0 =
                                    (1 - tau_current) * (gammma * (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) /
                                                                   (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                    std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                         (1 - gammma) * (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) /
                                                                         (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
                            ...
                            d_sellers_t_1 =
                                    tau_current * (gammma * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1)) / mu) /
                                                             (std::exp((a[1][0] - (theta[0] * p_sellers_t_1)) / mu) + exp_a_0_mu)) +
                                                   (1 - gammma) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1)) / mu) /
                                                                   (std::exp((a[1][1] - (theta[1] * p_sellers_t_1)) / mu) + exp_a_0_mu)))
                                    +
                                    (1 - tau_current) * (gammma * (std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) /
                                                                   (std::exp((a[0][0] - (theta[0] * p_sellers_t_0) - c[0]) / mu) +
                                                                    std::exp((a[1][0] - (theta[0] * p_sellers_t_1) - c[0]) / mu) + exp_a_0_mu)) +
                                                         (1 - gammma) * (std::exp((a[1][1] - (theta[1] * p_sellers_t_1) - c[1]) / mu) /
                                                                         (std::exp((a[0][1] - (theta[1] * p_sellers_t_0) - c[1]) / mu) +
...
//                        pi_plat_sum += pi_plat_t[j];
                ...
                for (int i{ 0 }; i < n; ++i)
                {
                    pi_sellers_store[i][e][w][tau]  = pi_sellers_sum[i] / results_check;
                    rvn_sellers_store[i][e][w][tau]  = rvn_sellers_sum[i] / results_check;
                    d_sellers_store[i][e][w][tau]  = d_sellers_sum[i] / results_check;
                    p_sellers_store[i][e][w][tau]  = p_sellers_sum[i] / results_check;
                }
                pi_plat_store[e][w][tau]  = pi_plat_sum/ results_check;
                cs_store[e][w][tau]  = cs_sum / results_check;
                for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                    a_plat_store[plat_action][e][w][tau]  = a_plat_sum[plat_action] / results_check;
...
//                std::vector<double> rvn_sellers_sum(n, 0.0);
            ...
            for (int e{ 0 }; e < E; ++e)
            {
                for (int i{ 0 }; i < n; ++i)
                {
                    pi_sellers_sum[i] += pi_sellers_store[i][e][w][tau] ;
//                        rvn_sellers_sum[i] += rvn_sellers_store[i][e][w][tau] ;
                    d_sellers_sum[i] += d_sellers_store[i][e][w][tau] ;
                    p_sellers_sum[i] += p_sellers_store[i][e][w][tau];
                }
                converge_sum += converge_store[e][w][tau];
//                    pi_plat_sum += pi_plat_store[e][w][tau];
                cs_sum += cs_store[e][w][tau];
                for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                    a_plat_sum[plat_action] += a_plat_store[plat_action][e][w][tau];
            }
            for (int i{ 0 }; i < n; ++i)
            {
                pi_sellers_store_avg_e[i][w][tau] = pi_sellers_sum[i] / E;
//                    rvn_sellers_store_avg_e[i][w][tau] = rvn_sellers_sum[i] / E;
                d_sellers_store_avg_e[i][w][tau] = d_sellers_sum[i] / E;
                p_sellers_store_avg_e[i][w][tau] = p_sellers_sum[i] / E;
            }
            converge_store_avg_e[w][tau] = converge_sum / E;
//                pi_plat_store_avg_e[w][tau] = pi_plat_sum/ E;
            cs_store_avg_e[w][tau] = cs_sum / E;
            for (int plat_action{ 0 }; plat_action < plat_actions; ++plat_action)
                a_plat_store_avg_e[plat_action][w][tau] = a_plat_sum[plat_action] / E;
    ...
    // Writing headers
    pi_sellers_file << "omega,tau,firm,profit\n";
    d_sellers_file << "omega,tau,firm,demand\n";
    p_sellers_file << "omega,tau,firm,price\n";
    cs_file << "omega,tau,cs\n";
    converge_file << "omega,tau,t\n";
    a_plat_file << "omega,tau,action,Count\n";
    ...
    for (int w{ 0 }; w < omega.size(); ++w)
    {
        for (int tau{ 0 }; tau < tau_values; ++tau)
        {
            ...
            for (int i{0}; i < n; ++i) 
            {
                pi_sellers_file << omega[w] << "," << tau_current << "," << i + 1 << "," << pi_sellers_store_avg_e[i][w][tau] << "\n";
                d_sellers_file << omega[w] << "," << tau_current << "," << i + 1 << "," << d_sellers_store_avg_e[i][w][tau] << "\n";
                p_sellers_file << omega[w] << "," << tau_current << "," << i + 1 << "," << p_sellers_store_avg_e[i][w][tau] << "\n";
            }
            cs_file << omega[w] << "," << tau_current << "," << cs_store_avg_e[w][tau] << "\n";
            converge_file << omega[w] << "," << tau_current << "," << converge_store_avg_e[w][tau]
                          << "\n";
            for (int j{0}; j < plat_actions; ++j)
            {
                a_plat_file << omega[w] << "," << tau_current << "," << j + 1 << "," << a_plat_store_avg_e[j][w][tau] << "\n";
            }
        }
    }
